...
#define REQ(s) (const char *)(s), (int32_t)strlen(s)

/* ok runs a data op expected to succeed, null-terminates the answer, and returns its length. */
static int32_t ok(lj_status_t (*op)(lj_handle_t, lj_cstr, int32_t, char *, int32_t, int32_t *),
                  lj_handle_t h, const char *req, char *out, int32_t cap) {
    int32_t len = -1;
...
int main(void) {
    ...
    int32_t before = ok(ic_lj_event_entries, h, entries_req, out, sizeof out);
    ...
    int32_t fb = ok(ic_lj_forest, h, "", forest_before, sizeof forest_before);
    int32_t nlen = -1;
    ...
    int32_t fa = ok(ic_lj_forest, h, "", forest_after, sizeof forest_after);
    ...
    char poll1[LJ_MUTATION_OUT_MIN + 4096]; int32_t p1 = -1;
    ...
    char poll2[LJ_MUTATION_OUT_MIN + 4096]; int32_t p2 = -1;
